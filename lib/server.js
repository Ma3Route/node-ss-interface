 ...
 * @param {Number} [config.max_size] - maximum size of a filled cache
 */
 ...
 *
 * @todo remove the <self._size> as it is unnecessary
 *
 * @public
 * @param {Function} done - called once query is done.
 */
...
Server.prototype.add = function add(items, callback) {
    ...
    for (var index = 0; index < len; index++) {
        item = items[index];
        args.push(item.id, JSON.stringify(item));
    }
